...
const PinCreate = ({ user }) => {
  ...
  const savePin = () => {
    if (title && about && destination && imageAsset?._id && category) {
      ...
      writeClient.create(doc).then(() => navigate('/'))
    } else {
      setFields(true)
      setTimeout(() => {
        setFields(false)
    ...
    <div className="flex flex-col justify-center items-center mt-5 lg:h-4/5">
      ...
      <div className="flex lg:flex-row flex-col justify-center items-center bg-white lg:p-5 p-3 lg:w-4/5 w-full">
        <div className="bg-secondaryColor p-3 flex flex-0.7 w-full">
          <div className="flex justify-center items-center flec-col border-2 border-dotted border-gray-300 p-3 w-full h-420">
            {loading && (
              <Spinner />
            )}
            {wrongImageType && (
              ...
              <label>
                <div className="flex flex-col items-center justify-center h-full">
                  ...
                  <p className="text-gray-400">
                    Use high-quality JPEG, SVG, PNG or GIF less than 20MB
                  </p>
                </div>
                <input 
                  type="file"
